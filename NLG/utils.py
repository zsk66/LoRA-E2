...
LoraLayer.stable_init = stable_init
...
def kaiming_init(self, adapter_name):
    ...
    if hasattr(self, "lora_bias"):
        if adapter_name in self.lora_bias and self.lora_B[adapter_name].bias is not None:
            self.lora_B[adapter_name].bias.data.zero_()
LoraLayer.kaiming_init = kaiming_init
...
def new_update_layer(
    ...
    if init_lora_weights == "stable_init":
        self.stable_init(adapter_name)
    elif init_lora_weights == "kaiming_init":
